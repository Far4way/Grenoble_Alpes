...
MatLed::MatLed() : pin(PIN_MATLED), nbLedx(NB_LED_X), nbLedy(NB_LED_Y), red_default(0), green_default(0), blue_default(0), posX(0), posY(0)
{
  FastLED.addLeds<WS2812, PIN_MATLED>(Tableds, NB_LED_X*NB_LED_Y);
}

/*affiche les leds dans l etat demande*/
uint8_t MatLed::show(uint8_t notuse){
  FastLED.show();
  return 1;
}
...
uint8_t MatLed::RGB_1Led(uint8_t notuse){
...
retourne 0 si echec, 1 si réussite*/
uint8_t MatLed::Black_1Led(uint8_t notuse){
  uint8_t numLed = coord_to_num(posX, posY);
  if(numLed != -1){
...
retourne 0 si echec, 1 si réussite*/
uint8_t MatLed::RGB_ligne(uint8_t notuse){
...
retourne 0 si echec, 1 si réussite*/
uint8_t MatLed::Black_ligne(uint8_t notuse){
  if(posY>=0 && posY<nbLedy){
    for(uint8_t i=0; i<nbLedx; i++){
...
retourne 0 si echec, 1 si réussite*/
uint8_t MatLed::RGB_column(uint8_t notuse){
...
retourne 0 si echec, 1 si réussite*/
uint8_t MatLed::Black_column(uint8_t notuse){
...
uint8_t MatLed::RGB_all(uint8_t notuse){
...
uint8_t MatLed::Black_all(uint8_t notuse){
  for(uint8_t i=0; i<nbLedy*nbLedx; i++){
